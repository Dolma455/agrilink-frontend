...
import MarketHubCard from "./MarketHubFarmerCardList"  // ← Your card component
...
export default function FarmerMarketHubPage() {
  ...
  const fetchSubmittedProposals = async () => {
    ...
    try {
      ...
      setSubmittedProposals(res.data.data || [])
    } catch (err) {
      console.error(err)
    }
  }
                ...
                {submittedProposals.map((prop) => (
                  <MarketHubCard
                    ...
                    order={{
                      ...prop,
                      id: prop.marketHubId,
                      productId: prop.marketHubId,
                      priceRangeMin: prop.offerPricePerUnit,
                      priceRangeMax: prop.offerPricePerUnit,
                      status: prop.status,
                    }}
                    imageUrl={productImages[prop.marketHubId] || "/placeholder.jpg"}
                    farmerId={farmerId}
                    onProposalSubmitted={() => {}}
                    isSubmitted={true}
                    submittedData={prop}
